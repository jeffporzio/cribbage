...
if __name__ == "__main__":
	...
	print("Size of look_up_dict: %i bytes"%(sys.getsizeof(look_up_dict)))
	
	start = time.time()		
	
	# Chunking variables
	nProcs = 8 # 4 on laptop, 8 on desktop (70 / 40, seconds respectively) 
	chunk_size = 1000000
	...
	while chunk_length_returned > 0:
		...
		with multiprocessing.Pool(nProcs) as pool:	
			chunk_scores = pool.map(getScoresOfHand,chunk)			
